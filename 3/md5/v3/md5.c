...
* DESCRIPTION:
*
* Process a block of bytes into a MD5 state structure. 64 bytes at a time.
*
* RETURNS:
*
* None.
...
// Memcpy for Data where Length is known to compile time	
...
* bytes.  
* A static analysis of this function yields a WCET of 17741 cycles
*
* RETURNS:
*
* None.
...
void md5_buffer_56(const char *buffer, const unsigned int buf_len, void *signature)
{
	...
	unsigned int len;
	unsigned int len_g = buf_len;
	unsigned int len_ng = WCET_LEN;
	
	len = (buf_len<= WCET_LEN) ? len_g : len_ng;
	
	/* process whole buffer but last buf_len % MD5_BLOCK bytes */
	MEMCPY(md5.md_buffer, buffer, md5.md_buffer, len, 1, MD5_BLOCK_SIZE);
	md5.md_buf_len = len;
