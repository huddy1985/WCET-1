...
*
* buf_len - The length of the buffer. (exactly MD5_BLOCK_SIZE
*/
static void process_block(md5_t *md5_p, const void *buffer, const unsigned int buf_len)
{
	...
	*/
	short g = md5_p->md_total[0] > MAX_MD5_UINT32 - MD5_BLOCK_SIZE;
	md5_uint32 total_1 = md5_p->md_total[1] + 1;
	md5_uint32 total_0 = md5_p->md_total[0] - (MAX_MD5_UINT32 + 1 - MD5_BLOCK_SIZE);
	md5_uint32 total_10 = md5_p->md_total[1];
 	md5_uint32 total_00 = md5_p->md_total[0] + MD5_BLOCK_SIZE;
...
#define MEMCPY(dest,src_true,src_false,n,guard,N) 		\
do {								\
	int nix = 0;						\
	for (int i = 0; i < (N); i++) {				\
		int j = i < (n) ? i : nix;			\
		char g = *(char*)((src_true)+j);		\
		char ng = *(char*)((src_false)+j);		\
		char res = guard ? g : ng;			\
		char def = *(char*)((dest)+i);			\
		*(char*)((dest)+i) = i < (n) ? res : def;	\
	}							\
}while (0)
...
* buffer - A buffer of bytes whose MD5 signature we are calculating.
*
* buf_len - The length of the buffer. (max. MD5_BLOCK_SIZE)
*/
void md5_process(md5_t *md5_p, const void *buffer, const unsigned int buf_len)
{
	...
	*/
	short g1 = md5_p->md_buf_len > 0;
	
	in_block = md5_p->md_buf_len;

	MEMCPY(md5_p->md_buffer + in_block, buffer, md5_p->md_buffer + in_block, len, g1, MD5_BLOCK_SIZE);
	md5_uint32 buf_len_g1 = md5_p->md_buf_len + len;
	md5_uint32 buf_len_ng1 = md5_p->md_buf_len;
	md5_p->md_buf_len = g1 ? buf_len_g1 : buf_len_ng1;
	...
	const void * buf_g1 = (const char *)buffer + len;
	const void * buf_ng1 = buffer;
	buffer = g1 ? buf_g1 : buf_ng1;
	
	unsigned int len_g1 = 0;
	unsigned int len_ng1 = len;
	len = g1 ? len_g1 : len_ng1;
	
	/* copy remaining bytes into the internal buffer */
	short g4 = len > 0;
	MEMCPY(md5_p->md_buffer, buffer, md5_p->md_buffer, len, g4, MD5_BLOCK_SIZE);
	md5_uint32 buf_len_g4 = len;
	md5_uint32 buf_len_ng4 = md5_p->md_buf_len;
	md5_p->md_buf_len = g4 ? buf_len_g4 : buf_len_ng4;
}
